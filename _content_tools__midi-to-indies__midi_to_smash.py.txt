#!/usr/bin/env python3
"""
MIDI drums â†’ Smash Drums Editor chart converter.
...
# Smash OS: charts live under ~/Charts
_charts = Path.home() / "Charts" / "Songs"
if _charts.is_dir():
    DEFAULT_OUTPUT = _charts
else:
    DEFAULT_OUTPUT = Path.home() / "Charts" / "Songs"
...
def copy_as_ogg(src: Path, dst: Path) -> None:
    ...
    # (game may only accept ogg â€” still package original for editor load)
    dst.write_bytes(src.read_bytes())
...
def convert_one(
    ...
    *,
    ...
    print(f"  Audio:    {audio_src if has_audio else '(none â€” load Song in editor)'}")
    print()
    ...
    for n, cnt in sorted(parsed["gm_counts"].items(), key=lambda x: -x[1]):
        ...
        print(f"    {n:3d} {GM_NAMES.get(n, 'unknown'):16s} x{cnt:<5d} â†’ {dest}")
    if parsed["unmapped"]:
    ...
    print()
    print("  Open in Smash Drums Editor â†’ Import â†’ pick the .indies file")
    if not has_audio:
...
def inspect_midi(path: Path) -> None:
    ...
    for n, cnt in sorted(parsed["gm_counts"].items(), key=lambda x: -x[1]):
        ...
        print(f"    {n:3d} {GM_NAMES.get(n, '?'):16s} x{cnt:<5d} â†’ {dest}")
    if parsed["unmapped"]:
